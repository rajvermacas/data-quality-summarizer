...
class AggregationMetrics:
    ...
    def update_latest_values(self, business_date: date, dataset_record_count: int,
                           filtered_record_count: int, level_of_execution: str):
...
class StreamingAggregator:
    ...
    def process_row(self, row: pd.Series):
        ...
        if business_date is None:
            logger.warning(f"Skipping row with invalid business_date: {row['business_date']}")
            return
    ...
    def _calculate_rolling_windows(self, key: Tuple[str, str, str, str, int], 
                                 reference_date: date):
        """
        ...
        logger.debug(f"Calculated rolling windows for key {key}: "
    ...
    def finalize_aggregation(self):
        ...
        logger.info(f"Finalizing aggregation with latest_business_date: {self.latest_business_date}")
    ...
    def get_accumulator_summary(self) -> Dict:
        ...
        return {
            ...
            'latest_business_date': str(self.latest_business_date) if self.latest_business_date else None,
            'total_entries': sum(len(metrics.row_data) for metrics in self.accumulator.values())
        }