...
Examples:
  ...
  python -m src input.csv rules.json --output-dir /custom/path
    ...
    parser.add_argument(
        'csv_file',
        help='Path to input CSV file containing data quality results'
    )
    
    parser.add_argument(
        'rule_metadata_file', 
        help='Path to JSON file containing rule metadata mappings'
    )
...
def run_pipeline(
    ...
    try:
        ...
        logger.info(f"Generated metrics for {unique_keys} unique dataset-rule combinations")
