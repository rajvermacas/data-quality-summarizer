...
class TestCLIArgumentParsing:
    ...
    def test_parse_args_with_custom_chunk_size(self):
        ...
        with patch('sys.argv', ['__main__.py', 'input.csv', 'rules.json', '--chunk-size', '50000']):
            args = main_module.parse_arguments()
    ...
    def test_parse_args_with_custom_output_dir(self):
        ...
        with patch('sys.argv', ['__main__.py', 'input.csv', 'rules.json', '--output-dir', '/custom/path']):
            args = main_module.parse_arguments()
...
class TestMainEntryPoint:
    ...
    def test_main_with_valid_arguments(self, mock_stdout, mock_stderr, monkeypatch, temp_files):
        ...
        with patch.object(main_module, 'run_pipeline', return_value=mock_result):
            with patch('sys.argv', ['__main__.py', temp_files['csv_file'], temp_files['rules_file']]):
    ...
    @patch('sys.stderr', new_callable=StringIO)
    @patch('sys.stdout', new_callable=StringIO)
    def test_main_with_pipeline_failure(self, mock_stdout, mock_stderr, monkeypatch, temp_files):
        """Test main function when pipeline fails."""
        mock_result = {
            'success': False,
            'error': 'File not found: test.csv'
        }
